...
class Action:
    ...
    def Shotgun(self,Shotgun_Bool): #각각 실탄일시 hp다운 시스템 및 총알은 index -1빠지도록
        if Shotgun_Bool==0:
            if self.bullet[-1]=="실탄":
            ...
            if self.bullet[-1]== "실탄":
                return #다른 객체에 전할값 너 피 깎아야한다. + 실탄, 공포탄
            
        self.shotgun_hp=1
        return "실탄" if self.bullet.pop() == "실탄" else "공포탄"
    
    # 모든 아이템 함수는 선택한 아이템 목록값을 0으로 변경합니다.
    def Handcuffs(self,inven_index): #턴 여부를 확인하는 리스트 [1]값을 1으로 변경합니다.
        self.turn_bool[1]=1
        self.inven[inven_index]=0
        
    def Beer(self,inven_index): #탄약 목록에 가장 마지막값를 반환하며 삭제합니다.
        self.inven[inven_index]=0
        return self.bullet.pop()

    def Magnifying_Glass(self,inven_index):#탄약 목록의 가장 마지막값을 반환합니다.
        self.inven[inven_index]=0
        return self.bullet[-1]

    def Cigarette_Pack(self,inven_index):#글로벌로 hp+1를 합니다.
        self.hp+=1
        self.inven[inven_index]=0

    def Hand_Saw(self,inven_index):#총hp값을 2로 고정합니다.
        self.Shotgun_hp=2
        self.inven[inven_index]=0


    def Burner_Phone(self,inven_index):#한발 남은 경우 제외 랜덤한 번수의 탄의 번수랑 극성을 알려준다.
        self.inven[inven_index]=0
    ...
    def Inverter(self,inven_index):#극성을 전환시켜준다.
        self.inven[inven_index]=0
        if self.bullet[-1]=="실탄":
            self.bullet[-1]="공포탄"
        else:
    ...
    def Expired_Medicine(self,inven_index):#각 50%확률로 2칸 올리기 or 한칸 소모
    ...
    def Adrenaline(self,inven_index,target,target_index,):#상대  아이템 삭제
        ...
        if inv_name in self.inven_list:
            result = self.inven_list[inv_name](target_index,2)
        self.inven[inven_index]=0
